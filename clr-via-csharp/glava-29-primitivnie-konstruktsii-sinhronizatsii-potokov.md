...
2. Использование Interlocked.CompareExchange\(\) вместо второй блокировки - если s\__value по прежнему null - то значение из временной переменной записывается в поле s\__value, если же кто-то первый записал, то замены не происходит.
