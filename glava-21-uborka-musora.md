...
2. Виндовз не хватает памяти \(CLR подписано на соответсвтущие события\)
3. Выгрузка домена
...
2. Но если превышение в поколении 2, то уборщик мусора увеличивает размер поколения 0, чтобы была возможность создать этот объект из-за которого возникает превышене и возобновляет работу потоков и позволяет создать объект из-за которого у нас возникает превышение в поколении 0
